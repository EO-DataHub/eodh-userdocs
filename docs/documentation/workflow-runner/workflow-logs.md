...
node_stage_in.log – logs from the STAGEIN step of the workflow, these can be useful if your inputs to the workflow are incorrect or not accessible to the Workflow Runner
...
Logs require authentication and authorization to view and only the workspace which initially requested the workflow execution, known as the calling workspace as described here, can view the associated status and logs.
