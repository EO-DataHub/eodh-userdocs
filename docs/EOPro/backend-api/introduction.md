...
**Integration with Telespazio's ADES API (OGC Processing API)**
A key feature of the Backend API is its integration with [Telespazio's ADES API](https://eoepca.readthedocs.io/projects/deploy/en/stable/) which allows for the execution of complex geospatial processes. This integration enables the Action Creator to leverage external computational resources, ensuring that users can perform sophisticated analyses on large datasets without
